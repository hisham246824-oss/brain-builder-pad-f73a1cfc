...
export default function PomodoroPage() {
  ...
  const {
    mode, minutes, seconds, totalSeconds, remainingSeconds,
    isRunning, changeMode, start, pause, reset,
  } = usePomodoro(settings.alarmSound);

  const getCurrentColor = () => {
    switch (mode) {
      case 'study': return getColorValue(settings.studyColor);
      case 'shortBreak': return getColorValue(settings.shortBreakColor);
      case 'longBreak': return getColorValue(settings.longBreakColor);
      default: return getColorValue(settings.studyColor);
    }
  };
    ...
    return (
      <div className="min-h-screen py-6 px-4">
        <div className="mx-auto max-w-md flex flex-col items-center">
          ...
          <div className="w-full"><MotivationalPhrases /></div>
        </div>
      </div>
    );
  }
